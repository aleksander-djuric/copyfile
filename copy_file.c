...
int copy_file(const char *src, const char *dst, int func, cp_callback cb_func) {
	...
	if (S_ISDIR(ss.st_mode)) {
		...
		if ((dir = opendir(src))) {
			...
			while ((d = readdir(dir))) {
				...
				if (copy_file(src_path, dst_path, func, cb_func) < 0)
					return -1;
			}
		}
		closedir(dir);
	...
	if (func == CP_MODE_MOVE) {
		...
		cps.curpos = cps.size + 1; // handle zero size files
...
int remove_file(const char *path, int func, cp_callback cb_func) {
	char fname[PATH_MAX];
	cp_state cps;
	struct stat st;
	int rc;
	...
	if (S_ISDIR(st.st_mode)) {
		...
		if ((dir = opendir(path))) {
			...
			while ((d = readdir(dir))) {
				...
				if (remove_file(fname, func, cb_func) < 0)
					return -1;
			}
		}
		closedir(dir);
