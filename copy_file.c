...
int remove_file(char *path, int func, cp_callback cb_func) {
	char fname[PATH_MAX];
	cp_state cps;
	struct stat st;
	int rc;
	...
	if (S_ISDIR(st.st_mode)) {
		...
		if ((dir = opendir(path))) {
			...
			while ((d = readdir(dir))) {
				...
				if ((rc = remove_file(fname, func, cb_func)) < 0)
					return rc;
			}
		}
		closedir(dir);
...
int copy_file(char *src, char *dst, int func, cp_callback cb_func) {
	...
	if (dst == NULL && func == CP_MODE_REMOVE)
		return remove_file(src, func, cb_func);
	...
	if (S_ISDIR(ss.st_mode)) {
		...
		if ((dir = opendir(src))) {
			...
			while ((d = readdir(dir))) {
				...
				if ((rc = copy_file(src_path, dst_path, func, cb_func)) < 0)
					return rc;
			}
		}
		closedir(dir);
	...
	for (buffer_size = cps.size;; buffer_size /= 2) {
		if (!(buffer = malloc(buffer_size))) {
			if (buffer_size >= getpagesize()) continue;
			else return -1;
		} else break;
	}
